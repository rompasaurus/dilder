/**
 * Joystick Mood Selector — Interactive mood browser with GPIO joystick input
 *
 * Builds on the Mood Selector firmware, adding 5-way joystick (DollaTek)
 * support via GPIO.  LEFT/RIGHT cycle through the 16 moods, UP picks a
 * random mood, DOWN picks a new random quote, CENTER resets to SASSY.
 * The last joystick input is shown on the right side of the display.
 * Serial input is still supported as a fallback.
 *
 * Display wiring (Pico-ePaper-2.13):
 *   VSYS -> VSYS     pin 39    GND  -> GND      pin 38
...
/* Debounce: minimum ms between joystick reads */
...
/* ─── Last input indicator (persists across frames) ─── */

static char last_input[12] = "";  /* e.g. "LEFT", "RIGHT", "UP", "DOWN", "CTR" */

/* ─── Mood selector status bar with input indicator ─── */

static void draw_status_bar(uint8_t current_mood) {
    /* Draw "< MOOD_NAME >" centered at the bottom of the screen */
    char status[32];
    snprintf(status, sizeof(status), "< %s >", mood_names[current_mood]);
    int status_len = (int)strlen(status);

    /* Center the mood label */
    ...
    /* Draw last input on the right side of the display */
    if (last_input[0]) {
        int input_len = (int)strlen(last_input);
        int input_w = input_len * 6;
        int input_x = IMG_W - input_w - 2;  /* 2px right margin */
        draw_text(input_x, status_y, last_input, IMG_W);
    }
}
...
static void print_help(void) {
    printf("\n");
    printf("========================================\n");
    printf("  JOYSTICK MOOD SELECTOR - INPUT OPTIONS\n");
    printf("========================================\n");
    printf("\n");
    printf("  JOYSTICK (GPIO):\n");
    printf("    LEFT   (GP4) = Previous mood\n");
    printf("    RIGHT  (GP5) = Next mood\n");
    printf("    UP     (GP2) = Random mood\n");
    printf("    DOWN   (GP3) = New random quote\n");
    printf("    CENTER (GP6) = Reset to Sassy\n");
    printf("\n");
    printf("  SERIAL (USB CDC):\n");
    printf("    [  or ,  = Previous mood (left)\n");
    printf("    ]  or .  = Next mood (right)\n");
    printf("    r        = Random mood\n");
    printf("\n");
    printf("  DIRECT MOOD SELECT (serial):\n");
...
static void joystick_init(void) {
    ...
    printf("Joystick GPIO initialized: UP=%d DOWN=%d LEFT=%d RIGHT=%d CTR=%d\n",
           JOY_UP, JOY_DOWN, JOY_LEFT, JOY_RIGHT, JOY_CENTER);
}
...
int main(void) {
    ...
    uint32_t last_joy_time = 0;  /* debounce timestamp */
    ...
    while (true) {
        ...
        printf("Frame %lu  |  < %s > (%d/%d)  |  input: %-6s  |  \"%s\"\n",
               (unsigned long)frame_idx,
               mood_names[current_mood],
               current_mood + 1, MOOD_COUNT,
               last_input[0] ? last_input : "-",
               quotes[qi].text);

        frame_idx++;

        /* Poll for joystick + serial input during frame delay (4 seconds) */
        uint32_t elapsed = 0;
        ...
        while (elapsed < frame_delay_ms && !input_received) {
            ...
            if (now - last_joy_time >= DEBOUNCE_MS) {
                if (gpio_get(JOY_LEFT) == 0) {
                    ...
                    snprintf(last_input, sizeof(last_input), "LEFT");
                    last_joy_time = now;
                    printf("  [JOY] LEFT  -> %s\n", mood_names[current_mood]);
                    mood_changed = true;
                    input_received = true;
                    ...
                    snprintf(last_input, sizeof(last_input), "RIGHT");
                    last_joy_time = now;
                    printf("  [JOY] RIGHT -> %s\n", mood_names[current_mood]);
                    mood_changed = true;
                    input_received = true;
                    ...
                    snprintf(last_input, sizeof(last_input), "UP");
                    last_joy_time = now;
                    printf("  [JOY] UP    -> %s (random)\n", mood_names[current_mood]);
                    mood_changed = true;
                    input_received = true;
                } else if (gpio_get(JOY_DOWN) == 0) {
                    qi = pick_quote_for_mood(current_mood);
                    snprintf(last_input, sizeof(last_input), "DOWN");
                    last_joy_time = now;
                    printf("  [JOY] DOWN  -> new quote\n");
                    mood_changed = true;
                    input_received = true;
                } else if (gpio_get(JOY_CENTER) == 0) {
                    current_mood = MOOD_NORMAL;
                    snprintf(last_input, sizeof(last_input), "CTR");
                    last_joy_time = now;
                    printf("  [JOY] CTR   -> reset to %s\n", mood_names[current_mood]);
                    mood_changed = true;
                    input_received = true;
                }
            }

            /* ── Serial input polling (50ms timeout) ── */
            ...
            if (ch != PICO_ERROR_TIMEOUT) {
                ...
                switch (ch) {
                    case '[': case ',':
                        current_mood = (current_mood == 0) ? MOOD_COUNT - 1 : current_mood - 1;
                        input_action = (ch == '[') ? "[" : ",";
                        snprintf(last_input, sizeof(last_input), "SER:%s", input_action);
                        break;
                    case ']': case '.':
                        current_mood = (current_mood + 1) % MOOD_COUNT;
                        input_action = (ch == ']') ? "]" : ".";
                    ...
                    case 'r': case 'R':
                        current_mood = rng_next() % MOOD_COUNT;
                        snprintf(last_input, sizeof(last_input), "SER:R");
                        break;
                    case 'n': case 'N': current_mood = MOOD_NORMAL;    snprintf(last_input, sizeof(last_input), "SER:N"); break;
                    case 'w': case 'W': current_mood = MOOD_WEIRD;     snprintf(last_input, sizeof(last_input), "SER:W"); break;
                    case 'u': case 'U': current_mood = MOOD_UNHINGED;  snprintf(last_input, sizeof(last_input), "SER:U"); break;
                    case 'a': case 'A': current_mood = MOOD_ANGRY;     snprintf(last_input, sizeof(last_input), "SER:A"); break;
                    case 's': case 'S': current_mood = MOOD_SAD;       snprintf(last_input, sizeof(last_input), "SER:S"); break;
                    case 'c': case 'C': current_mood = MOOD_CHAOTIC;   snprintf(last_input, sizeof(last_input), "SER:C"); break;
                    case 'h': case 'H': current_mood = MOOD_HUNGRY;    snprintf(last_input, sizeof(last_input), "SER:H"); break;
                    case 't': case 'T': current_mood = MOOD_TIRED;     snprintf(last_input, sizeof(last_input), "SER:T"); break;
                    case 'p': case 'P': current_mood = MOOD_SLAPHAPPY; snprintf(last_input, sizeof(last_input), "SER:P"); break;
                    ...
                    case 'q': case 'Q':
                        qi = pick_quote_for_mood(current_mood);
                        snprintf(last_input, sizeof(last_input), "SER:Q");
                        mood_changed = true;
                        break;
                    case '?':
                        print_help();
